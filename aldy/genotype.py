...
def genotype(
    ...
    if avg_cov < 2:
        raise AldyException(
            f"Average coverage of {avg_cov:.2f} for gene {gene.name} is too low; skipping gene {gene.name}. "
            + f"Please ensure that {gene.name} is present in the input SAM/BAM."
        )
    elif avg_cov < 20:
        log.warn(
            f"Average sample coverage is {avg_cov}. We recommend at least 20x coverage for the best results."
        )
    ...
    # Add SLACK to each score to avoid division by zero or other numerical issues when the optimum
    # is close to zero.
    # HACK: Value of 1 (1 gene copy) seems to be reasonable, but is not theoretically guaranteed
    # to be better than any other value.
    SLACK = 1
    ...
    for i, cn_sol in enumerate(cn_sols):
        ...
        log.info(
            f"      Confidence: {(min_cn_score + SLACK) / (cn_sol.score + SLACK):.2f} (score = {cn_sol.score:.2f})"
        )
    log.info("")
    ...
    for i, major_sol in enumerate(major_sols):
        ...
        log.info(
            f"      Confidence: {(min_major_score + SLACK) / (major_sol.score + SLACK):.2f} (score = {major_sol.score:.2f})"
        )
    log.info("")
    ...
    for i, minor_sol in enumerate(minor_sols):
        ...
        log.info(f"      Minor: {t}")
        log.info(
            f"      Confidence: {(min_minor_score + SLACK) / (minor_sol.score + SLACK):.2f} (score = {minor_sol.score:.2f})"
        )
        if output_file:
