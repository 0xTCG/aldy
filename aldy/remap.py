...
def realign(alleles, tempdir, sam_path):
    def makeref():
        ...
        ref = Fasta('/home/frashidi/database/genome/hg19/genome.fa')
        fake_alleles = {}
        fake_alleles['6'] = str(ref['chr22'][42519175:42528416]).upper()
        fake_alleles['7'] = str(ref['chr22'][42531290:42545394]).upper()
        for i in '67':
            with open('{}/{}.fa'.format(tempdir, i), 'w') as f:
                print('>{}'.format(i), file=f)
                # print('\n'.join(textwrap.wrap(alleles[i][1])), file=f)
                print('\n'.join(textwrap.wrap(fake_alleles[i])), file=f)
    ...
    def realign(path, regions):
        ...
        for i in '67':
            ...
            # out = cmd('mrfast -e 2 --search {0}/{1}.fa --seq {2} -o {2}.{1}.t.sam --best'.format(tempdir, i, newpath))
            # out = cmd('cat {1}.{0}.t.sam | head -n -2 > {1}.{0}.sam'.format(i, newpath))
            
            # out = cmd('bwa mem -h 12 {0}/{1}.fa {2} > {2}.{1}.sam'.format(tempdir, i, newpath))
            # out = cmd('bwa mem {0}/{1}.fa {2} > {2}.{1}.sam'.format(tempdir, i, newpath))
...
@timing
def remap(sam_path, gene, sam, cn_sol, out_dir, tempdir=None, force=True, cleanup=True, remap_mode=1):
    if tempdir is None:
        tempdir = tempfile.mkdtemp(suffix='_tmpaldy', dir=out_dir)
    log.critical('Temp is ' + tempdir)

    if str(remap_mode) == '2':
        output = remap_farid.remap(sam_path, gene, sam, cn_sol, tempdir)
        out = out_dir+'/{}.remapped.bam'.format(os.path.basename(sam_path)[:-len('.bam')])
        cmd('mv {} {}'.format(output, out))
    else:
