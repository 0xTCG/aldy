...
def optimize(gene, reads, alleles, copy_number):
   ...
   for r in reads:
      for a in reads[r]:
         x[r][a] = model.model.addVar(vtype='B', name='x_{}_{}'.format(a, r))

   # Should we include the read or not?
   for r in reads:
      x[r][-1] = model.model.addVar(vtype='B', name='d_{}'.format(r))
   ...
   for r in reads:
      for a in reads[r]:
         read = reads[r][a][0]
         for k in xrange(read.reference_start, read.reference_end):
            z[a, k] += x[r][a]
   for (a, k), lhs in z.iteritems():
      a_start = alleles[a][0][0]
      region = gene.region_at[a_start + k]
      assert(region != '')
      z[a, k] = model.model.addVar(lb=-model.INF, name='z_{}_{}'.format(a, k))
      model.model.addConstr(z[a, k] == lhs - copy_number[region] * copy_number.baseline[a_start + k])
   model.update()

   # Objective
   def score(r, a):
      if not r[a][0].has_tag('NM'):
         return 0
      return len(r[a][0].query_sequence) - r[a][0].get_tag('NM')
...
def remap(sam_path, gene, sam, cn_sol, tempdir='temp'):
   def sequence(gene, id, pad=0):
      regs = [b for a, b in gene.regions.items() if a.startswith(str(id) + '.')]
      mi, ma = min(r[0] for r in regs), max(r[0] for r in regs)
      st = gene.region[1]
   ...
   def makeref(gene):
      for i in '67':
         with open('{}/{}.fa'.format(tempdir, i), 'w') as f:
            print >>f, '>{}'.format(i)
            print >>f, '\n'.join(textwrap.wrap(alleles[i][1]))
   ...
   def realign(path):
      ...
      cmd('samtools fastq {} > {}'.format(path, newpath))
      for i in '67':
   ...
   out = '{}/{}.new.bam'.format(tempdir, os.path.basename(sam_path)) 
