...
class Mutation(nt('Mutation', ['pos', 'op', 'functional', 'aux'])):
   ...
   """
   def __new__(self, pos, op, functional=0, aux=dict(), **kwargs):
...
class Gene(object):
   ...
   def sequence(self, id, pad=0):
      regs = [b for a, b in self.regions.items() if a.startswith(str(id) + '.')]
      mi, ma = min(r[0] for r in regs), max(r[0] for r in regs)
      st = self.region[1]
      return self.seq[max(0, mi - st - pad):min(ma - st + pad, len(self.seq))]
