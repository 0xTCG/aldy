   ...
   for ref_id in '67':
      ...
      with pysam.AlignmentFile('{}/{}.fq.{}.sam'.format(tempdir, os.path.basename(sam_path), ref_id)) as sam:
         for read in sam.fetch():
            ...
            for k in range(read.reference_start, read.reference_end):
               which_read_at[ref_id, k].append(read)
   return reads, which_read_at


@timing
def remap(sam_path, gene, sam, cn_sol, tempdir=None, force=True, cleanup=True, remap_mode=1):
   if tempdir is None:
   ...
   if str(remap_mode) == '2':
      ...
      reads, which_read_at = get_reads(alleles, tempdir, sam_path)
      ...
      n = 10
      chunks = {}
      for a,v in alleles.items():
         t = tuple([i-v[0][0]+1 for i in v[0]])
         chunks[a] = get_chunks(range(t[0], t[1]), n)
      ...
      for r1,r2 in zip(chunks['6'], chunks['7']):
         ...
         for i, j in zip(r1, r2):
            if which_read_at.has_key(('6',i)):
               for x in which_read_at['6',i]:
                  temp_reads[x.query_name]['6'] = reads[x.query_name]['6']
            if which_read_at.has_key(('7',j)):
               for x in which_read_at['7',j]:
                  temp_reads[x.query_name]['7'] = reads[x.query_name]['7']
         log.warn('Optimizing {}/{}...'.format(counter, n))
         temp_reads = optimize(gene, temp_reads, reads, alleles, cn_sol)
         for x in temp_reads:
            final_reads[x] = temp_reads[x]
      ...
      write_reads(sam_path, gene, alleles, final_reads, out)
