...
def pytest_generate_tests(metafunc):
   ...
   if "real_sample" in metafunc.fixturenames and not path:
      ...
      for file, data in read_data('aldy.tests.paper/data-pgx1.json').items():
         samples.append((data, f'{path}/cdc/pgrnseq-v1/bams/{file}.cram', 'pgrnseq-v1', f'{path}/cram-genome.fa'))
      for file, data in read_data('aldy.tests.paper/data-pgx2.json').items():
         samples.append((data, f'{path}/baylor/pgrnseq-v2/bams/{file}.cram', 'pgrnseq-v2', f'{path}/cram-genome.fa'))
      for file, data in read_data('aldy.tests.paper/data-illumina.json').items():
         samples.append((data, f'{path}/1000genomes-illumina/bams/{file}.bam', 'illumina', None))
