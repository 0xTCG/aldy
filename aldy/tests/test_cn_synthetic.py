...
def assert_cn(gene, expected, cov, expected_obj=None, gap=0):
   ...
   for s in sols:
      assert (s.score - best_sol_score * (1 + gap)) < SOLUTION_PRECISION, "Gap violation"
...
def test_basic(toy_gene):
   ...
   assert_cn(toy_gene,
             [{'1': 2}],
             make_coverage(toy_gene, zip([1.8,2.2, 2.1,1.9, 1.7], [2.05,1.95, 2,2, 2.7])),
...
def test_multiplication(toy_gene):
