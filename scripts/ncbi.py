...
def blat(a, b):
    ...
    ret = os.system(
        "./blat _a _b _out -out=pslx -stepSize=5 -repMatch=2253 -minScore=0 -minIdentity=0 -fine >/dev/null"
    )
    if ret != 0:
        raise "BLAT failed"
    with open("_out") as f:
...
def get_pseudo_mutations(gene, pseudogene, force=False):
    ...
    for ei, (e6, e7) in enumerate(
        zip(gene.exons, pseudogene.exons) + zip(gene.introns, pseudogene.introns)
    ):
        ...
        log.debug(
            "NCBI: ALN {}",
            yy(
                "".join(["*" if s[0][y] != s[2][y] else "-" for y in xrange(len(s[0]))])
            ),
        )
...
def get_genomes(
    ...
    handle = Entrez.read(
        Entrez.esearch(
            db="nucleotide", term=" ".join(g[1] for g in gene_ids), retmode="xml"
        )
    )
    for gi, gid in enumerate(handle["IdList"]):
        ...
        genome = Entrez.efetch(
            db="nucleotide", id=gid, rettype="gb", retmode="text", **params
        ).read()
        ...
        log.trace(
            "NCBI: Gene {} BLAT results: hit {}, query {}",
            genome.id,
            alignment.hit_range,
            ...
            if reverse_complement:
                exons = [
                    SeqFeature.FeatureLocation(
                        len(genome.seq) - e.end, len(genome.seq) - e.start, 1
                    )
