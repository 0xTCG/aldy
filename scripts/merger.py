    ...
    for e in gene.exons:
        ...
        if e.start <= m["pos"] < e.end:
            ...
            if seq[m["pos"] - e.start] != m["op"][4]:
                log.debug(
                    "Merge: Karolinska's SNP {} does not match reference {}",
                    m,
                    seq[m["pos"] - e.start],
                )
            s = seq.tomutable()
            ...
            if neigh != None:
                ...
                for n in neigh:
                    if (
                        isinstance(n["pos"], int)
                        and (n != m)
                        and (nt + e.end - n["pos"]) / 3 == codon
                    ):
                        log.debug(
                            "Merge: {}:{} and {}:{} are in same codon".format(
                                m["pos"], m["op"], n["pos"], n["op"]
                            )
                        )
    ...
    for a in sorted(result):
        ...
        for mi, m in enumerate(result[a]["mutations"]):
            ...
            if op == "A6":  # hack
                ...
                if op[-1] in "+-":
                    ...
                    if len(parts) == 2:
                        start, end = tuple(
                            map(
                                lambda x: coordinate(int(x), gene), parts[1].split("-")
                            )[::-1]
                        )
                    else:
                        if op[0] == "e":
                    ...
                    mutations += [
                        copy.deepcopy(gene.pseudo_mutations[i])
                        for i in xrange(start, end)
                        if i in gene.pseudo_mutations
                    ]
                continue
            if pos < 0:
                pos += 1
            if op[:3] == "ins":
                ...
                if reverse_complement:
                    op = "SNP.{}{}".format(
                        Seq(op[0]).reverse_complement(), Seq(op[2]).reverse_complement()
                    )
                else:
                    op = "SNP.{}{}".format(Seq(op[0]), Seq(op[2]))
            mutations.append(
                dict(
                    pos=coordinate(pos, gene),
                    op=op,
                    dbsnp=db,
                    old="{}:{}".format(m[0], m[1]),
                )
        ...
        for m in mutations:
            m["functional"] = is_functional(
                gene, m, exceptions, reverse_complement, mutations
            )
