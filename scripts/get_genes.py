...
def process_mut_helper(m, dbsnp):
    result = []
    rs = re.findall(
        r"\s*?([-0-9_]+)\s*?((ins|del|dup|[ACGT])(\S*))", " " + m
    )  # Match 100C>T, -100C>T, -100 C>T etc.
    if len(rs) == 0:
        return None
    for r in rs:
            ...
            for a in col.find_all("a"):
                ...
                if r is not None:
                    ...
                    if snp in dbsnp and dbsnp[snp] != rs:
                        print >> sys.stderr, "error {} != {} for {}".format(
                            rs, dbsnp[rs], snp
                        )
                    dbsnp[snp] = rs
            # split mutations
...
def get_data(
    ...
    result = merger.merge(
        cypdata, genes[gene], coordinate, functional_exceptions, reverse_complement
    )
    ...
    for a in result:
        for m in result[a]["mutations"]:
            ...
            if m["pos"] not in genes[gene].translation:
                log.warn(
                    "Main: Translation not found for {}: {} ({})", a, m["old"], m["pos"]
                )
                m["pos"] = None
            else:
    ...
    for _, g in genes.iteritems():
        g.exons[:] = map(
            lambda x: (g.translation[int(x.start)], g.translation[int(x.end)]), g.exons
        )
        g.introns[:] = map(
    ...
    for pos, m in genes[gene].pseudo_translation.iteritems():
        hoi[genes[gene].translation[pos]] = NoIndent(
            (genes[pseudogene].translation[m["old_pos"]], m["op"] if "op" in m else "")
        )
    return dict(
        # map=hoi,
        seq=hg19,
        ...
        name=gene,
        exons={
            "{}".format(ei + 1): NoIndent(e) for ei, e in enumerate(genes[gene].exons)
        },
        special_regions={
            g: NoIndent(gg) for g, gg in genes[gene].special_regions.iteritems()
        },
        pseudogenes={
            g: {
                "exons": {
                ...
                "special_regions": {
                    g: NoIndent(gg) for g, gg in genes[g].special_regions.iteritems()
                },
            }
            for g in [pseudogene]
        }
        if pseudogene is not None
        else {},
                ...
                (
                    a,
                    {
                        ...
                        "mutations": [
                            NoIndent(
                                OrderedDict(
                                    [(x, y[x]) for x in sorted(y, reverse=True)]
                                )
                            )
...
def get_cyp2d6(force):
    ...
    def process_mut(m, dbsnp):
        if pseudogene not in m:
            ...
            if op[0] not in "ie":
                return None  # only introns/exons
            if "onwards" in m:
                op += "+"
            r = re.search(r"\((\d+)-(\d+)\)", m)
            if r is not None:
                op += ":{}-{}".format(r.group(1), r.group(2))
            return [[pos, op, dbsnp]]
    ...
    def fix_karolinska(cypdata):
        cypdata[gene + "*82"]["mutations"] = [
            m for m in cypdata[gene + "*82"]["mutations"] if m[0] != "pseudogene"
        ]
        ...
        extra_alleles = [
            m
            for m in cypdata[gene + "*4N"]["mutations"]
            if "{}:{}".format(m[0], m[1])
            in "4401:C>T 3384:A>C 3582:A>G 2097:A>G 997:C>G 843:T>G 746:C>G 310:G>T -1000:G>A -1235:A>G".split()
        ]
        for i in "4A 4B 4C 4D 4E 4F 4G 4H 4J 4K 4L".split():
            cypdata[gene + "*" + i + "W"] = copy.deepcopy(cypdata[gene + "*" + i])
            cypdata[gene + "*" + i + "W"]["mutations"] += [
                f
                for f in extra_alleles
                if f not in cypdata[gene + "*" + i + "W"]["mutations"]
            ]

        extra_alleles = [
            m
            for m in cypdata[gene + "*2A"]["mutations"]
            if "{}:{}".format(m[0], m[1])
            in "-1584:C>G -1235:A>G -740:C>T -678:G>A".split()
        ]
        extra_alleles += [
            m
            for m in cypdata[gene + "*4N"]["mutations"]
            if "{}:{}".format(m[0], m[1]) in "310:G>T 746:C>G 843:T>G".split()
        ]
        ...
        extra_alleles += [
            m
            for m in cypdata[gene + "*36"]["mutations"]
            if "{}:{}".format(m[0], m[1])
            in "-1235:A>G -1000:G>A 310:G>T 843:T>G 3582:A>G 3384:A>C".split()
        ]
        ...
        extra_alleles = [
            ...
            for m in cypdata[gene + "*36"]["mutations"]
            if "{}:{}".format(m[0], m[1])
            in "-1235:A>G 310:G>T 843:T>G 3384:A>C".split()
        ]
        for i in "17".split():
            cypdata[gene + "*" + i + "W"] = copy.deepcopy(cypdata[gene + "*" + i])
            cypdata[gene + "*" + i + "W"]["mutations"] += [
    ...
    def post_process(genes, result):
        ...
        result[gene + "*77"] = result.pop(
            gene + "*13G1"
        )  # 77 is same as 13; ignoring 77
        ...
        for i in ["36", "57", "4N", "83"]:
            i = gene + "*" + i
            result[i]["mutations"] = [
                m for m in result[i]["mutations"] if m["old"] != "4180:G>C"
            ]
    ...
    return get_data(
        ...
        [
            "224384747",
            (gene, "M33388.1"),
            (pseudogene, "M33387.1", {"seq_start": 7000}),
        ],
        coordinate,
        [],
        post_process,
...
def get_cyp2a6(force):
    ...
    def fix_karolinska(cypdata):
        ...
        cypdata.update(
            {
                gene
                + "*3": {  # This one is retention in e3, e6 and e8 (check original paper)
                    "mutations": {
                        ("pseudogene", "e3:1-151", ""),
                        ("pseudogene", "e6:1-143", ""),
                        ("pseudogene", "e8:1-143", ""),
    ...
    return get_data(
        ...
        fix_karolinska,
        hg19_range,
        ["224384750", (gene, "NG_008377.1"), (pseudogene, "NG_007960.1")],
        coordinate,
        [(51, "C")],
        post_process,
...
def get_cyp2c19(force):
    ...
    def fix_karolinska(cypdata):
        ...
        extra_alleles = [
            m
            for m in cypdata[gene + "*17"]["mutations"]
    ...
    return get_data(
        ...
        [
            ...
            (
                gene,
                "NC_000010.9",
                {"seq_start": 96822172, "seq_stop": 96783519, "strand": 2},
            ),
        ],
        coordinate,
        [],
    ...
    return get_data(
        ...
        [
            ...
            (
                gene,
                "NG_000004.3",
                {"seq_start": 240000, "seq_stop": 272000, "strand": 1},
            ),
        ],
        coordinate,
        [(6986, "T"), (31611, "G")],
...
def get_cyp3a4(force):
    ...
    return get_data(
        ...
        [
            "224589819",
            (gene, "AF280107.1", {"seq_start": 60000, "seq_stop": 90000, "strand": 1}),
        ],
        coordinate,
        [],
        post_process,
...
def main(force=False):
    for n, f in globals().iteritems():
        if not n.startswith("get_") or n == "get_data":
            continue
        if n == "get_cyp2c8":
            continue
        if n == "get_cyp4f2":
